...
func (me *lexer) next()        { me.token = me.scan.Scan() }
func (me *lexer) text() string { return me.scan.TokenText() }

type lexPanic string

// describe returns a string describing the current token, for use in errors.
func (me *lexer) describe() string {
	switch me.token {
	case scanner.EOF:
		return "end of file"
	case scanner.Ident:
		return fmt.Sprintf("identifier %s", me.text())
	case scanner.Int, scanner.Float:
		return fmt.Sprintf("number %s", me.text())
	}
	return fmt.Sprintf("%q", rune(me.token)) // any other rune
}
